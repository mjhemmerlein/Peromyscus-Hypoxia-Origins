...
# Combined Analysis  ------------------
LP_JZ_Sample_Info <- read_xlsx("RNA_Seq_RawData/MetaData_LP.xlsx")
...
Check = LP_JZ_Sample_Info$JZ_Seq_Name
colnames(Pman_rawreads) == Check

colnames(Pman_rawreads) = rownames(LP_JZ_Sample_Info)
...
# All hypoxia sensitive genes

LP_ISO_Summary = read_xlsx("RNA_Seq_Output/LP_JZ_ISO_Ortho_Summary.xlsx")
...
annotation_col <- data.frame(
  Group = LP_JZ_Sample_Info$Group,
  Sex = LP_JZ_Sample_Info$Sex
)
...
pheatmap(
  ...
  gaps_row = gaps_row,
  gaps_col = gaps_col,
  main = "LP JZ Z-scores of Unique O2/IXN-Significant Genes",
...
hm = pheatmap(
  ...
  gaps_row = gaps_row,
  gaps_col = gaps_col,
  main = "LP JZ_Z-scores of Unique O2/IXN-Significant Genes",
  show_rownames = ifelse(nrow(z_scores) < 50, TRUE, FALSE),
  color = viridis(100, option = "A")
)

save_pheatmap_pdf(hm, "Plots/Heatmaps/LP_JZ_Zscore_heatmap.pdf")
