...
OXPHOS = intersect(PBS$gene_mus, leading_edge_long$leadingEdge)
...
DNA_rep = intersect(PBS$gene_mus, leading_edge_long$leadingEdge)
...
MYC = intersect(PBS$gene_mus, leading_edge_long$leadingEdge)

PBS_Lowland <- data.frame(
  pathway = c(rep("OXPHOS", length(unique(OXPHOS))),
              rep("DNA_rep", length(unique(DNA_rep))),
              rep("MYC", length(unique(MYC)))),
  gene = c(unique(OXPHOS), unique(DNA_rep), unique(MYC))
)
...
# HIGHLAND ONLY ------
# Ranked genes
rankings_high <- sign(EP_ISO_Strain$ME_O2_logFC)*(-log10(EP_ISO_Strain$ME_O2_P.Val)) # signed p values from spatial DGE as ranking
